		...
		jQuery(function($){
			...
			$('.action-button').click(function(){
				...
				if (action == 'install'){
					...
					$status.html('<?php _e( 'Installing...', 'us' ); ?>');
				} else {
					$tile.addClass('status_activating');
					$status.html('<?php _e( 'Activating...', 'us' ); ?>');
				}
				$.ajax({
					type: 'POST',
					url: '<?php echo admin_url('admin-ajax.php'); ?>',
					...
					success: function(data) {
						...
						if (data != undefined && data.success) {
							...
							$status.html('<?php _e( 'Installed & Activated', 'us' ); ?>');
						} else {
							$tile.addClass('status_error');
							if (data != undefined && data.data != undefined && data.data.message != undefined) {
								$status.html(data.data.message);
							} else {
								$status.html('<?php echo  us_translate_with_external_domain( 'An error has occurred. Please reload the page and try again.' ); ?>');
							}
						}
					}
				});
